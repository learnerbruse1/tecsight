# Third-Party Notices

TecSight 集成了以下第三方开源组件：

## LibreHardwareMonitor
- 来源 Source: <https://github.com/LibreHardwareMonitor/LibreHardwareMonitor>
- 许可证 License: **Mozilla Public License 2.0 (MPL-2.0)**
- 版权 Copyright: LibreHardwareMonitor contributors
- 用途 Used for: 温度 / 风扇 / 电压传感器读数与磁盘 SMART 健康度
- MPL-2.0 全文: <https://www.mozilla.org/en-US/MPL/2.0/>
- 说明：LibreHardwareMonitor 源文件头部的许可声明予以保留；本项目未修改其文件，仅以 NuGet 包形式集成。

LibreHardwareMonitor 的以下传递依赖同样随发布产物分发：

## BlackSharp.Core
- 来源 Source: <https://github.com/Blacktempel/BlackSharp>
- 许可证 License: **MPL-2.0**
- 版权 Copyright: Florian K.
- 用途 Used for: LibreHardwareMonitor 依赖的通用基础组件

## DiskInfoToolkit
- 来源 Source: <https://github.com/Blacktempel/DiskInfoToolkit>
- 许可证 License: **MPL-2.0**
- 版权 Copyright: Florian K.
- 用途 Used for: 磁盘信息与 SMART 数据读取

## RAMSPDToolkit-NDD
- 来源 Source: <https://github.com/Blacktempel/RAMSPDToolkit>
- 许可证 License: **MPL-2.0**
- 版权 Copyright: Florian K.
- 用途 Used for: 内存 SPD 与温度数据读取

## HidSharp
- 来源 Source: <https://software.seekye.com/hidsharp>
- 许可证 License: **Apache License 2.0**
- 版权 Copyright: HidSharp contributors
- 用途 Used for: HID 设备枚举与 VID/PID 识别
- Apache-2.0 全文: <https://www.apache.org/licenses/LICENSE-2.0>

## Vanara.PInvoke.Kernel32
- 来源 Source: <https://github.com/dahall/Vanara>
- 许可证 License: **MIT**
- 版权 Copyright: David Hall and contributors
- 用途 Used for: 系统运行时长、内存、电池状态、UEFI Secure Boot 等 Kernel32 原生 API

## Vanara.PInvoke.DwmApi
- 来源 Source: <https://github.com/dahall/Vanara>
- 许可证 License: **MIT**
- 版权 Copyright: David Hall and contributors
- 用途 Used for: 窗口标题栏深浅色主题（DWM immersive dark mode）

## ManagedNativeWifi
- 来源 Source: <https://github.com/emoacht/ManagedNativeWifi>
- 许可证 License: **MIT**
- 版权 Copyright: © 2015 emoacht
- 用途 Used for: Wi-Fi 接口与当前连接信息（SSID/BSSID/信号/信道/速率）
